        ...
        <div className="space-y-4">
          {items.map((item, index) => (
            ...
            >
              ...
              >
                <span
                  className={`font-bold text-lg ${openIndex === index ? "text-primary" : "text-slate-800"}`}
                >
                  {item.question}
                </span>
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${openIndex === index ? "bg-primary text-white" : "bg-slate-100 text-slate-500"}`}
                >
                  {openIndex === index ? (
                    <Minus size={16} />
                  ) : (
